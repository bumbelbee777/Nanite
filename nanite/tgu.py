...
class TokenGenerationUnit(nn.Module):
    ...
    def __init__(
        ...
        # Token prediction head - make it complex-valued
        self.token_head = nn.Sequential(
            nn.Linear(d_model, d_model * 2, dtype=torch.complex64),
            ComplexPReLU(d_model * 2),  # Use complex-safe activation with correct dim
            nn.Dropout(dropout),
            nn.Linear(d_model * 2, d_model, dtype=torch.complex64),
        )
    ...
    async def forward(
        ...
        async with self.latent_lock:
            ...
            token_input = self.latent_state[:, -1]
            if not torch.is_complex(token_input):
                token_input = torch.complex(token_input, torch.zeros_like(token_input))
            token_pred = self.token_head(token_input)
