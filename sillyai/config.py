    ...
    def __post_init__(self):
        ...
        if Modality.IMAGE in self.supported_modalities:
            if self.image_size <= 0:
                raise ValueError("image_size must be positive when IMAGE modality is supported")
        ...
        if not all(isinstance(x, int) for x in [self.pad_token_id, self.bos_token_id, self.eos_token_id, self.unk_token_id]):
