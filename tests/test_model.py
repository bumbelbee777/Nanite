...
class TestTransformerComponents:
    def test_positional_encoding(self):
        config = ModelConfig(d_model=64, max_seq_len=32, device="cpu")
    ...
    def test_transformer_layer(self):
        config = ModelConfig(d_model=64, n_heads=4, d_ff=128, dropout=0.1, device="cpu")
        ops = MultivectorOps()
    ...
    def test_complex_mlp(self):
        config = ModelConfig(d_model=64, d_ff=128, dropout=0.1, device="cpu")
        ops = MultivectorOps()
...
class TestComplexLoss:
    def test_complex_loss_with_real_target(self):
        ...
        pred = torch.complex(torch.randn(16, 32, 64), torch.randn(16, 32, 64))
    ...
    def test_complex_loss_with_complex_target(self):
        ...
        pred = torch.complex(torch.randn(16, 32, 64), torch.randn(16, 32, 64))
        target = torch.complex(torch.randn(16, 32, 64), torch.randn(16, 32, 64))
