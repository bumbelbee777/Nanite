...
from nanite.config import ModelConfig
from nanite.configs.large import LargeModelConfig
from nanite.configs.medium import MediumModelConfig
...
class PartialSolver(Nanite):
    ...
    def __init__(
        ...
        self.checkpoint_interval = 100  # Save checkpoint every 100 sentences
        ...
        # Initialize Wikipedia crawler (lazy initialization)
        self.crawler = None
    ...
    def _compile_model(self, mode: str = "reduce-overhead"):
        ...
        torch.compile(self, mode=mode)

        # Collect modules to compile
        modules_to_compile = [(name, module) for name, module in self.named_modules() if isinstance(module, (nn.Module, nn.ModuleList, nn.ModuleDict))]

        # Compile and set them
        for name, module in modules_to_compile:
            try:
        ...
        # Compile key methods
        torch.compile(self.forward, mode=mode)
        torch.compile(self.generate_response, mode=mode)
        torch.compile(self.process_text, mode=mode)

        # Compile modality manager
        torch.compile(self.modality_manager, mode=mode)

        # Compile tokenizer
        if hasattr(self, "tokenizer"):
            torch.compile(self.tokenizer, mode=mode)
    ...
    def _save_checkpoint(self, loss: float, is_final: bool = False) -> None:
        ...
        checkpoint_path = self.checkpoint_dir / f"potato_{prefix}_{timestamp}.pt"
        ...
        try:
            ...
            self.logger.info(f"Saved {prefix} to {checkpoint_path}")
            
            # Update best loss if this is better
            if loss < self.best_loss:
                self.best_loss = loss
                # Save a copy as the best model
                best_path = self.checkpoint_dir / "potato_best.pt"
                torch.save({
                    'model_state_dict': self.state_dict(),
                    'optimizer_state_dict': self.optimizer.state_dict() if hasattr(self, 'optimizer') else None,
                    'loss': loss,
    ...
    def load_latest_checkpoint(self):
        ...
        checkpoints = list(self.checkpoint_dir.glob("potato_checkpoint_*.pt"))
        if not checkpoints:
            return
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"Resumed with loss: {self.best_loss:.4f}")
            except Exception as e:
                self.logger.warning(
    ...
    def save_checkpoint(self):
        """Save a checkpoint."""
        if not hasattr(self, 'current_loss'):
            return
            
        self._save_checkpoint(self.current_loss)
    ...
    def save_final_checkpoint(self):
        ...
        self._save_checkpoint(self.current_loss, is_final=True)
